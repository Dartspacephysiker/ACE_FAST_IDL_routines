...
FUNCTION get_chaston_ind,maximus,satellite,lun,DBFILE=dbfile,CDBTIME=cdbTime,CHASTDB=CHASTDB, $
  ...
  IF ~KEYWORD_SET(maxNegMC) THEN maxNegMC=defMaxNegMC
  ...
  IF NOT KEYWORD_SET(dbfile) AND NOT KEYWORD_SET(CHASTDB) THEN BEGIN
     ...
     IF FILE_TEST(cdbTimeFile) AND N_ELEMENTS(cdbTime) EQ 0 THEN restore,cdbTimeFile ELSE PRINTF,lun,"cdbTimeFile already provided! Not loading default..."
  ...
  IF maximus EQ !NULL THEN restore,loaddataDir + dbfile ELSE BEGIN 
     PRINTF,lun,"There is already a maximus struct loaded! Not loading " + defDBFile
  ENDELSE
  ...
  ENDIF ELSE BEGIN
     ...
     ENDIF ELSE BEGIN
        ...
        ENDIF ELSE ind_ilat=where(maximus.ilat GE minI AND maximus.ilat LE maxI,n_ilat,NCOMPLEMENT=n_not_ILAT)
     ENDELSE
  ENDELSE
  ...
  IF KEYWORD_SET(dayside) THEN BEGIN
     dayside_i = WHERE(maximus.mlt GE 0.0 AND maximus.mlt LE 18.0,NCOMPLEMENT=n_outside_MLT)
     ind_region=cgsetintersection(ind_ilat,dayside_i)

     PRINTF,lun,"Only dayside!"
     PRINTF,lun,"n events on dayside: " + STRCOMPRESS(N_ELEMENTS(dayside_i),/REMOVE_ALL)
  ENDIF ELSE BEGIN

     ;;special treatment for nightside
     IF KEYWORD_SET(nightside) THEN BEGIN
        nightside_i = WHERE(maximus.mlt LE 6.0 OR maximus.mlt GE 18.0,NCOMPLEMENT=n_outside_MLT)
        ind_region=cgsetintersection(ind_ilat,nightside_i)
        
        PRINTF,lun,"Only nightside!"
        PRINTF,lun,"n events on nightside: " + STRCOMPRESS(N_ELEMENTS(nightside_i),/REMOVE_ALL)
     ENDIF ELSE BEGIN
  ...
  ind_magc_geabs10=where(maximus.mag_current LE maxNegMC OR maximus.mag_current GE minMC,NCOMPLEMENT=n_magc_LT_abs10)
