...
restore,dataDir + "/processed/idl_wind_and_acedata.dat"

;sort these, since they're not combined nicely
sortMag=SORT(mag_utc)
bx=bx(sortMag)
by=by(sortMag)
bz=bz(sortMag)
...
;where to save data
culledDataStr=dataDir + "/processed/culled_wind_ACE_magdata.dat"
...
by=by(sortMag)
bz=bz(sortMag)
bxy_over_bz=bxy_over_bz(sortMag)
cone_overclock=cone_overclock(sortMag)
mag_utc=mag_utc(sortMag)
phiclock=phiclock(sortMag)
thetacone=thetacone(sortMag)
