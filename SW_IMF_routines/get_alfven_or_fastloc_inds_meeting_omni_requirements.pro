  ...
  FOR iDel=0,NIter-1 DO BEGIN
     ...
     IF new_sweeter_messord THEN BEGIN
        ...
        IF n_streaks GT 0 THEN BEGIN
           ...
           checkStart = VALUE_LOCATE(startT,dbTimes[db_i]-nightDelay[db_i]) > 0 < (N_ELEMENTS(startT)-1);checkStart = VALUE_LOCATE(dbTimes[db_i],startT)
           checkStop  = VALUE_LOCATE(stopT,dbTimes[db_i]-nightDelay[db_i])  > 0 < (N_ELEMENTS(startT)-1);checkStop  = VALUE_LOCATE(dbTimes[db_i],stopT)
           gaps       = stopT-startT
           ...
           PRINT,FORMAT='("(Min,Med,Mean,Max) streakLen: (",I0,",",I0,",",I0,",",I0,") min")', $
                 MIN(gaps)/60., $
                 MEDIAN(gaps)/60., $
                 MEAN(gaps)/60., $
                 MAX(gaps)/60.
