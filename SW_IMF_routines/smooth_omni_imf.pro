  ...
  IF ~KEYWORD_SET(C_OMNI__is_smoothed) OR KEYWORD_SET(C_OMNI__RECALCULATE) THEN BEGIN
     ...
     FOR streakNum=0,C_OMNI__nStreaks-1 DO BEGIN
        ...
        IF N_ELEMENTS(byMin) GT 0 OR N_ELEMENTS(byMax) GT 0 THEN BEGIN 
           C_OMNI__By[curStart:curStop]        = SMOOTH(C_OMNI__By[curStart:curStop],smooth_IMF,/EDGE_TRUNCATE)
        ENDIF
     ...
     C_OMNI__thetaCone                         = ACOS(ABS(C_OMNI__Bx)/SQRT(C_OMNI__Bx*C_OMNI__Bx+C_OMNI__By*C_OMNI__By+C_OMNI__Bz*C_OMNI__Bz))*180/!PI
