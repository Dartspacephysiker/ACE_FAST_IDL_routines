...
;   ctrAngle=90 & $
   negAngle=angleLim1 & $
   posAngle=angleLim2 & $
ENDIF ELSE IF clockStr EQ 'dawnward' THEN BEGIN & $ 
;   ctrAngle=-90 & $
   negAngle=-angleLim2 & $
   posAngle=-angleLim1 & $
ENDIF ELSE IF clockStr EQ 'bzNorth' THEN BEGIN & $
;   ctrAngle=0 & $
   negAngle=-angleLim1 & $
   posAngle=angleLim1 & $
...
IF Bx_over_ByBz_Lim GT 0 THEN BEGIN & $
   Printf,Lun,"Print dat bx" & $
ENDIF

IF stableIMF EQ 0 AND Bx_over_ByBz_Lim EQ 0 THEN BEGIN & $
   printf,lun,"No stability or Bx requirement..." & $
ENDIF ;ELSE BEGIN & $
