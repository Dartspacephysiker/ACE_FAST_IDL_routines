/*
**	$Id: //depot/Release/ENVI52_IDL84/idl/idldir/external/call_external/Fortran/unix/tauest.c#1 $
**
** NAME:
** 	tauest
**
...
**
**      IDL> result = CALL_EXTERNAL('tauest.so', '_tauest',    $
...
**	is performed in the Fortran function tauest.	
**
** SIDE EFFECTS:
**	None.
**
...
;;
        ...
        result = CALL_EXTERNAL('tauest.so', '_tauest',     $
...
#if defined(SPARC) || defined(OSF1) || defined(__x86_64__)
   void tauest_f_();
#else
   void tauest_f();  /* function prototype for the fortran funct */
#endif

/*
** Declare variables
*/
   char         *orbStr;        /* Pointer to a char ( One byte )       */
   char         *itrvlStr;      /* Pointer to a char ( One byte )       */
   char         *detrend;       /* Pointer to a char ( One byte )       */
   char         *time_or_age;   /* Pointer to a char ( One byte )       */
...
*/
   ...
   */
      fprintf(stderr,"tauest_c: Incorrect number of arguments\n");
...
*/
   if( (return_string=(char*)malloc((unsigned)ch_size+1))
       == (char*)NULL)
   {
      fprintf(stderr,"tauest_c: malloc error \n");
...
#if defined(SPARC) || defined(OSF1) || defined(__x86_64__)
   tauest_f_(byte_var, short_var, long_var, float_var, double_var, 
	 string_var->s, return_string, &ch_size, STR_LEN(string_var),ch_size );
#else
   tauest_f(byte_var, short_var, long_var, float_var, double_var,
         string_var->s, return_string, &ch_size, STR_LEN(string_var),ch_size );
#endif
