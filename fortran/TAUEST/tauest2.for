...
c
      ...
      character*60 name
      parameter (file1='tauest1.tmp',file2='tauest2a.tmp',
     +           file3='tauest2a.plt',file4='tauest2b.tmp',
     +           file5='scatter.dat',file6='tauest2b.plt',
     +           detrn='Detrending:           no',
...
200   print *
     ...
     +        't:                  time [t]'
      print *,'                     age [a]'
      read (5,'(a)') answ1
      if (answ1.ne.'t'.and.answ1.ne.'a') go to 200
     ...
     +      access='sequential', status='old')
      if (answ1.eq.'t') then
         do 210 i=1,n
            ...
            x(i)=x(i)*1.0d0
210      continue
      else if (answ1.eq.'a') then
         do 220 i=1,n
            read (1,*) t(n+1-i),x(n+1-i)
            t(n+1-i)=-1.0d0*t(n+1-i)
            x(n+1-i)=1.0d0*x(n+1-i)
220      continue
      end if
      close (unit=1,status='keep')
c
...
300   print *
      ...
      read (unit=5,fmt='(a)') answ1
      if (answ1.ne.'n'.and.answ1.ne.'m'.and.answ1.ne.'l') go to 300
      if (answ1.eq.'m') then
         call avevar(x,n,b1fit,b2fit)
         do 310 i=1,n
            x(i)=x(i)-b1fit
310      continue
      else if (answ1.eq.'l') then
