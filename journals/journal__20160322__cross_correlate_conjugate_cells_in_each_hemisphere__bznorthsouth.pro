;2016/03/22 Can you show me what it's all about?
PRO JOURNAL__20160322__CROSS_CORRELATE_CONJUGATE_CELLS_IN_EACH_HEMISPHERE__BZNORTHSOUTH

  nSmooths          = 2
  nSmoothPoints     = 8
  hemiArr           = ['NORTH','SOUTH']

  ;; omniParamStr      = 'OMNI_params--bzSouth--negAngle_135__posAngle_-135--ABS_byMax_4.00.sav'
  omniParamStr      = 'OMNI_params--bzSouth--negAngle_135__posAngle_-135--ABS_byMax_5.00--ABS_bzMin_5.00.sav'

  inDir             = '/SPENCEdata/Research/Cusp/ACE_FAST/journals/journal__20160322__nev_per_orb_and_crosscorrelation_savefiles'
  inFile            = 'journal__20160322__CrossCorr_nev_per_orb_vs_delay--'+hemiArr+'--n_delays__4001--' $
                      + omniParamStr

  smoothStr         = STRCOMPRESS(nSmooths,/REMOVE_ALL) + "_smooths--"
  smoothNiceStr     = STRCOMPRESS(nSmooths,/REMOVE_ALL) + " Smooths"
  ...
  bzSouth_alfvenic_list   = LIST(smoothNEvList_hemis[0,2],smoothNEvList_hemis[1,3])
  bzSouth_particle_list   = LIST(smoothNEvList_hemis[0,3],smoothNEvList_hemis[1,2])
  ...
  bzSouth_alfvenic_titles = [title_list_hemis[0,2],title_list_hemis[1,3]]
  bzSouth_particle_titles = [title_list_hemis[0,3],title_list_hemis[1,2]]
  ...
  outPlot = "journal__20160322--BZNORTH_IMF--Alfvenic_and_particle--"+smoothStr $
  ...
  SET_PLOT_DIR,plotDir,/FOR_SW_IMF,/ADD_TODAY
  outPlot = "journal__20160322--BZSOUTH_IMF--Alfvenic_and_particle--"+smoothStr $
            + "--n_delays__" + STRCOMPRESS(nDelays,/REMOVE_ALL)+"--" + omni_paramStr+'.png'
  PRINT,'saving ' + outPlot
  window.save,plotDir+outPlot
  ...
  bzNorth_particle_ccor = C_CORRELATE(bzNorth_particle_list[0],bzNorth_particle_list[1],lags)
     ...
     outPlot_CC="journal__20160322--BZNORTH_IMF--Alfvenic_and_particle_hemi_CrossCor--" + smoothStr $
                + "--n_delays__" + STRCOMPRESS(nDelays,/REMOVE_ALL)+"--" + omni_paramStr + '.png'
  ...
  bzSouth_particle_ccor = C_CORRELATE(bzSouth_particle_list[0],bzSouth_particle_list[1],lags)
     ...
     outPlot_CC="journal__20160322--BZSOUTH_IMF--Alfvenic_and_particle_hemi_CrossCor--" + smoothStr $
                + "--n_delays__" + STRCOMPRESS(nDelays,/REMOVE_ALL)+"--" + omni_paramStr + '.png'
     PRINT,'saving ' + outPlot_CC
     window_cc.save,plotDir+outPlot_CC
