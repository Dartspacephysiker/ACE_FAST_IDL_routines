...
PRO JOURNAL__20160322__SMOOTH_THE_SMOOTHED_NEV_PER_ORB_DATA__THEN_PLOT
  ...
  nResmooths       = 1
  resmoothStr      = STRCOMPRESS(nResmooths,/REMOVE_ALL) + "_resmooths--"
  resmoothNiceStr  = STRCOMPRESS(nResmooths,/REMOVE_ALL) + " Resmooths ("+hemi+")"
  nSmoothPoints    = 32

  hemi             = 'NORTH'
  hemi             = 'SOUTH'

  inDir            = 'Research/Cusp/ACE_FAST/journals/journal__20160322__nev_per_orb_and_crosscorrelation_savefiles'
  inFile           = 'journal__20160322__CrossCorr_nev_per_orb_vs_delay--'+hemi+'--n_delays__4002--' + $
                     'OMNI_params--duskward--negAngle_45__posAngle_135--ABS_byMin_4.00--bzMax_-1.00.sav'
  ...
  WINDOW_CUSTOM_SETUP,NPLOTCOLUMNS=N_ELEMENTS(clockStr), $
                      ...
                      XTITLE='Delay (min)', $
                      YTITLE='N Events', $
                      WINDOW_TITLE=resmoothNiceStr, $
                      CURRENT_WINDOW=window
  
  resmoothedNEvList = LIST()
  FOR i=0,N_ELEMENTS(nEvArrList)-1 DO BEGIN
     resmoothedNEvList.add,SMOOTH(smoothednEvList[i],nSmoothPoints,/EDGE_TRUNCATE)

     resmooths      = 0
     WHILE resmooths LE nResmooths-1 DO BEGIN
        resmoothedNEvList[i] = SMOOTH(resmoothedNEvList[i],nSmoothPoints,/EDGE_TRUNCATE)
        resmooths++
     ENDWHILE
  ENDFOR

  xRange           = [delayArr[0],delayArr[-1]]/60.
  yRangeMin        = MIN(LIST_TO_1DARRAY(resmoothedNEvList,/SKIP_NEG1_ELEMENTS,/WARN),MAX=yRangeMax)

  FOR i=0,N_ELEMENTS(nEvArrList)-1 DO BEGIN
     plot          = PLOT(delayArr/60.,resmoothedNEvList[i], $
                          ;; TITLE=title_list[i], $
                          XRANGE=xRange, $
  ...
  outPlot = "journal__20160322__plot_nev_per_orb_vs_delay--"+resmoothStr $
  ...
  dawnward_ccor = C_CORRELATE(resmoothedNEvList[0],resmoothedNEvList[1],lags)
  duskward_ccor = C_CORRELATE(resmoothedNEvList[2],resmoothedNEvList[3],lags)
     ...
     outPlot_CC="journal__20160322__CrossCorr_nev_per_orb_vs_delay--" + resmoothStr $
                + hemi + "--n_delays__" + STRCOMPRESS(nDelays,/REMOVE_ALL)+"--" + omni_paramStr + '.png'
